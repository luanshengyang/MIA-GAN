...
from MI_cGAN_Generator import Generator
from MI_cGAN_Discriminator import Discriminator
...
# data path
path = "../../Data_Generation/Gan_Data/0.8/Gan_40_dBIndoor2p4_64ant_32users_8pilot.mat"
...
def generated_image(model, test_input, tar, t=0):
    ...
    for i in range(3):
        ...
        plt.axis("off")
    plt.savefig(os.path.join("generated_img", "40_4D_img_" + str(t) + ".png"))
...
def train(epochs):
    ...
    for epoch in range(epochs):
        ...
        mse.append(fuzz.mse(np.squeeze(realim), np.squeeze(prediction)))
        ...
        if epoch == epochs - 1:
            ...
            mse_epoch = TemporaryFile()
            np.save(nmse_epoch, nmse)
            np.save(mse_epoch, mse)

            # Save the predicted Channel
        matfiledata = {}  # make a dictionary to store the MAT data in
        matfiledata[u'predict_MIGan_4D_40dB_Indoor2p4_64ant_32users_8pilot'] = np.array(
        ...
        hdf5storage.write(matfiledata, '.',
                          'Results\Eest_MIGAN_' + str(epoch + 1) + '_4D_40db_Indoor2p4_64ant_32users_8pilot.mat',
        ...
        plt.xlabel('Epoch')
        plt.ylabel('NMSE')
        figurepath = "shiyanjieguo/{}.png".format(str(epoch) + '_4D_40db_64ant_32users_8pilot_NMSE')
...
if __name__ == "__main__":
    # train
    nmse, mse, ep = train(epochs=20)

    np.savetxt('NMSE_MIGan_4D_40_dB_Indoor2p4_64ant_32users_8pilot loss.txt', nmse, fmt='%15f')
    np.savetxt('MSE_MIGan_4D_40_dB_Indoor2p4_64ant_32users_8pilot loss.txt', mse, fmt='%15f')
    plt.figure()
    plt.plot(ep, nmse, '^-r')
    plt.xlabel('Espoch')
    plt.ylabel('NMSE')
    plt.title('Normalized Mean Squared Error (NMSE)')
    plt.show();